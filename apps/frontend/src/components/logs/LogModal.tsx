...
export function LogModal({ service, onClose }: { service: string | null; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/55 flex items-center justify-center" onClick={onClose}>
      <div
        className="w-[78%] max-w-[700px] bg-[#0a0f1a] border border-[#243049] rounded-2xl overflow-hidden shadow-2xl"
          ...
          <div className="flex items-center gap-1.5">
            {LEVELS.map((l) => (
              <button
                key={l}
                onClick={() => setFilter(l)}
                className={`text-[9px] px-2 py-1 rounded ${filter === l ? 'bg-[#15233d] text-[var(--primary)]' : 'text-[var(--faint)]'}`}
              >
                {l}
              </button>
            ))}
            <button
              title="tam ekran (Loglar)"
              onClick={() => {
                onClose();
