...
export default function Register() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!username || !email || !password) { setErr(t('auth.allFieldsRequired')); return; }
  ...
  if (done) return (
    <AuthShell>
      <div className="rounded-[var(--radius)] border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm text-center space-y-3">
        <p className="text-[var(--foreground)]">{t('auth.registrationDone')}</p>
        <Link to="/login" className="text-[var(--primary)] underline">{t('auth.backToLogin')}</Link>
      </div>
    </AuthShell>
  );
  return (
    <AuthShell>
      ...
      >
        <h1 className="text-xl font-bold text-[var(--heading)] text-center">{t('auth.registerTitle')}</h1>
        ...
        <label className="block text-sm text-[var(--muted)]">{t('auth.email')}
        ...
        <Link to="/login" className="block text-center text-sm text-[var(--primary)] hover:underline">{t('auth.haveAccount')}</Link>
      </form>
    </AuthShell>
  );
}
